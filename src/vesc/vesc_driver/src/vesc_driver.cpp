...
void VescDriver::speedCallback(const std_msgs::Float64::ConstPtr& speed)
{
  if (driver_mode_ = MODE_OPERATING) {
    if (speed->data == 0){
      vesc_.setSpeed(speed_limit_.clip(0.0));
      vesc_.setDutyCycle(duty_cycle_limit_.clip((speed->data)/-1000.0));
    }
    else if(speed->data > 0){
      vesc_.setSpeed(speed_limit_.clip(1800.0));
      vesc_.setDutyCycle(duty_cycle_limit_.clip((((speed->data)*0.6)+60.0)/-1000.0));
    }
    else{
      vesc_.setSpeed(speed_limit_.clip(-1800.0));
      vesc_.setDutyCycle(duty_cycle_limit_.clip((((speed->data)*0.6)-60.0)/-1000.0));
