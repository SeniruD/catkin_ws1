...
from robot_util.msg import robot_ctr
...
class JoyTeleop:
    ...
    def __init__(self):
        ...
        self.old_buttons = []
        self.autonomous_control = [0.0, 0.0, 0.0]  # [speed, steering]
        ...
        rospy.Subscriber('robot_control_cmd', robot_ctr, self.update_autonomous_control)

        ##########################################
        # sub to autonomous mtr and servo topics #
        ##########################################

        # Run a low-freq action updater
        rospy.Timer(rospy.Duration(2.0), self.update_actions)
    
    def update_autonomous_control(self, data):
        self.autonomous_control[1] = data.motor_speed
        self.autonomous_control[2] = data.steer_angle
            ...
            if c == 'autonomous_control':
                for mapping in cmd['axis_mappings']:
                    if len(joy_state.axes)<=mapping['axis']:
                        ...
                        if mapping['axis'] == 2:
                            val = self.autonomous_control[mapping['axis']] * mapping.get('scale', 1.0) + mapping.get('offset', 0.0)
                        else:
                            val = self.autonomous_control[mapping['axis']] * mapping.get('scale', 1.0) + mapping.get('offset', 0.0)
